<?php
/**
 * MK-MSG - Automação No Prazo (CLI Only)
 */
...
// Garantir que $diasnoprazo é um array
if (!is_array($diasnoprazo)) {
...
foreach ($diasnoprazo as $diasnoprazoAtual) {
    $queryNoPrazo = "SELECT upper(vtab_titulos.nome_res) as nome_res,
                     ...
                     WHERE DATEDIFF(vtab_titulos.datavenc, CURRENT_DATE()) = $diasnoprazoAtual
                     AND (vtab_titulos.status = 'aberto')
                     AND (vtab_titulos.cli_ativado = 's')
                     GROUP BY vtab_titulos.uuid_lanc
                     ORDER BY nome_res ASC;";
    
    $result = $conn->query($queryNoPrazo);
    
    if (!$result || $result->num_rows === 0) {
        echo "[" . date('Y-m-d H:i:s') . "] Sem registros para o dia $diasnoprazoAtual.\n";
        continue;
    }

    echo "[" . date('Y-m-d H:i:s') . "] Processando Dia $diasnoprazoAtual (" . $result->num_rows . " registros)...\n";
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $dir = "$rootPath/logs/$month/noprazo";
        ...
        $logData = sprintf("%s;%s;%s;%s;Dia:%d\n", date("d-m-Y"), date("H:i:s"), $nome, $err ?: $response, $diasnoprazoAtual);
