...
// Função para obter o próximo dia útil (ou o próprio dia se for útil)
...
foreach ($diasnoprazo as $dias) {
    // Se o vencimento cair em FDS/Feriado, antecipa o envio para o dia útil anterior.
    ...
    echo "[" . date('Y-m-d H:i:s') . "] Analisando títulos para regra de $dias dia(s) antes...\n";

    $jsonFile = __DIR__ . '/db/messages/noprazo.json';
    $msg = "";
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Data teórica de envio (X dias antes do vencimento)
        $dataEnvioTeorica = date('Y-m-d', strtotime("-$dias days", strtotime($dataVenc)));
        
        // Data de envio real (ajustada para dias úteis)
        $dataEnvioReal = $dataEnvioTeorica;
        
        if (isset($ignorar_fds_feriado) && $ignorar_fds_feriado == 1) {
            // Se a data teórica não for útil, precisamos antecipar para o dia útil anterior
            while (!isDiaUtil($dataEnvioReal)) {
                $dataEnvioReal = date('Y-m-d', strtotime("-1 day", strtotime($dataEnvioReal)));
            }
            
            // Caso especial: Se o vencimento for segunda e o envio for 1 dia antes (domingo),
            // Enviar na própria segunda.
            if ($dias == 1) {
                $diaSemanaVenc = date('N', strtotime($dataVenc));
                if ($diaSemanaVenc == 1) { // Segunda
                    $dataEnvioReal = $dataVenc;
                }
            }
        }
        ...
        echo "[" . date('H:i:s') . "] Enviado para: $nome | Vencimento: " . $row['datavenc_fmt'] . " | Status: " . ($err ? "Erro: $err" : "OK") . "\n";
