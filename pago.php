...
if (isset($_POST['ajax_send']) || isset($_POST['get_all_ids'])) {
    ...
    if (isset($_POST['ajax_send'])) {
        ...
        if (file_exists($jsonFile)) {
            ...
            $msgpago = $jsonData['content'] ?? "";
        }

        $nome = $contato['nome_res'] ?? 'N/A';
        $celular = $contato['celular'] ?? '';
        $datavenc = $contato['datavenc'] ?? '';
        $datapag = $contato['datapag'] ?? '';
        $linhadig = $contato['linhadig'] ?? '';
        $qrcode = $contato['qrcode'] ?? '';

        $buscar = ['/%provedor%/', '/%nomeresumido%/', '/%vencimento%/', '/%pagamento%/', '/%linhadig%/', '/%copiacola%/', '/%site%/'];
        $substituir = [$provedor, $nome, $datavenc, $datapag, $linhadig, $qrcode, $site];
    ...
    if (isset($_POST['get_all_ids'])) {
        ...
        $sql_todos = "SELECT upper(vtab_titulos.nome_res) as nome_res, 
                      ...
                      DATE_FORMAT(vtab_titulos.datavenc,'%d/%m/%y') AS datavenc, DATE_FORMAT(vtab_titulos.datapag,'%d/%m/%y') AS datapag, 
                      vtab_titulos.linhadig, sis_qrpix.qrcode 
                      FROM vtab_titulos 
                      LEFT JOIN sis_qrpix ON vtab_titulos.uuid_lanc = sis_qrpix.titulo 
                      WHERE DATE_FORMAT(datapag,'%m-%Y') = ? AND vtab_titulos.status = 'pago' AND vtab_titulos.cli_ativado = 's'
...
$where_clause = "WHERE DATE_FORMAT(datapag,'%m-%Y') = ? AND vtab_titulos.status = 'pago' AND vtab_titulos.cli_ativado = 's'
                 AND (vtab_titulos.deltitulo = 0 OR vtab_titulos.deltitulo IS NULL)
                 AND vtab_titulos.nome_res IS NOT NULL AND TRIM(vtab_titulos.nome_res) <> ''
                 AND vtab_titulos.celular IS NOT NULL AND TRIM(vtab_titulos.celular) <> ''
                 AND vtab_titulos.linhadig IS NOT NULL AND TRIM(vtab_titulos.linhadig) <> ''";
...
<div class="container">
    <div class="card mb-3">
        <h2 class="title-pago">✅ Títulos Pagos</h2>
    ...
    <div class="menu card mb-3">
        <div style="display: flex; gap: 12px; flex-wrap: wrap;">
            ...
            <button class="button2" onclick="location.href='pago.php'" type="button" style="background-color: var(--success); border: 2px solid var(--success);">✅ Pagos</button>
            ...
            <table class="custom-table w-100">
                <thead>
                    <tr>
                        ...
                        <th><a href="?menumes=<?= $valorsel ?>&search=<?= $search ?>&limit=<?= $limit ?>&order_by=datapag&order_dir=<?= ($order_by == 'datapag' && $order_dir == 'asc') ? 'desc' : 'asc' ?>">PAG. <?= $order_by == 'datapag' ? ($order_dir == 'asc' ? '↑' : '↓') : '' ?></a></th>
                        <th style="width: 50px; text-align: center;"><input type="checkbox" id="select_all" class="check"></th>
                    </tr>
                </thead>
                <tbody>
                    <?php if ($result->num_rows === 0): ?>
                        <tr><td colspan="5" class="text-center" style="padding: 20px;">Nenhum registro encontrado.</td></tr>
                    <?php else: ?>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr data-id="<?= $row['uuid_lanc'] ?>" data-nome="<?= htmlspecialchars($row['nome_res']) ?>" data-celular="<?= $row['celular'] ?>" data-venc="<?= $row['datavenc'] ?>" data-pag="<?= $row['datapag'] ?>" data-linha="<?= $row['linhadig'] ?>" data-qr="<?= $row['qrcode'] ?>">
                                ...
                                <td><span class="badge" style="background-color: #e6f7ed; color: #28a745;"><?= $row['datapag'] ?></span></td>
...
<script>
const STORAGE_KEY = 'mkmsg_selected_pago';
let isStopped = false;
...
async function processarEnvios(lista) {
    ...
    for (let i = 0; i < lista.length; i++) {
        ...
        try {
            const res = await $.ajax({ url: 'pago.php?menumes=<?= $valorsel ?>', method: 'POST', data: { ajax_send: true, contato }, dataType: 'json' });
...
$(document).ready(() => {
    ...
    $('.row-check').on('change', function() {
        ...
        const id = tr.data('id');
        const s = getSelected();
        if (this.checked) s[id] = { nome_res: tr.data('nome'), celular: tr.data('celular'), datavenc: tr.data('venc'), datapag: tr.data('pag'), linhadig: tr.data('linha'), qrcode: tr.data('qr') };
        else delete s[id];
        saveSelected(s);
    });
    ...
    $('#btn-todos').on('click', () => {
        if (confirm('Enviar para TODOS os <?= $total_registros ?> registros?')) {
            $.post('pago.php?menumes=<?= $valorsel ?>', { get_all_ids: true }, (data) => processarEnvios(data));
        }
    });
