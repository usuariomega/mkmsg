...
$stmt = $conn->prepare("SELECT upper(vtab_titulos.nome_res) as nome_res, REGEXP_REPLACE(vtab_titulos.celular,'[( )-]+','') AS celular, DATE_FORMAT(vtab_titulos.datavenc,'%d/%m/%y') AS datavenc, vtab_titulos.linhadig, sis_qrpix.qrcode FROM vtab_titulos INNER JOIN sis_qrpix ON vtab_titulos.uuid_lanc = sis_qrpix.titulo WHERE DATE_FORMAT(datavenc,'%m-%Y') = ? AND vtab_titulos.status = 'vencido' AND vtab_titulos.cli_ativado = 's' ORDER BY nome_res ASC, datavenc ASC");
...
function enviarMensagem($contato, $msgvencido, $vars, $wsip, $token, $tempomin, $tempomax) {
...
<div id="overlay" class="overlay">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['posttodos']) || isset($_POST['postsel']))) {
        ...
        if (isset($_POST['posttodos'])) {
            foreach ($dados as $contato) { if ($contato[0] !== 'Vazio') enviarMensagem($contato, $msgvencido, $vars, $wsip, $token, $tempomin, $tempomax); }
            ...
            foreach ($_POST['check'] as $index => $value) {
                if ($value == "1") {
                    ...
                    enviarMensagem($contato, $msgvencido, $vars, $wsip, $token, $tempomin, $tempomax);
                }
            }
        }
