    ...
    <form id="form" method="post">
        ...
        <div class="pagination mt-3">
            <?php for ($i = 1; $i <= $total_paginas; $i++): ?>
                <a href="?search=<?= $search ?>&limit=<?= $limit ?>&order_by=<?= $order_by ?>&order_dir=<?= $order_dir ?>&page=<?= $i ?>" class="page-link <?= $page == $i ? 'active' : '' ?>"><?= $i ?></a>
            <?php endfor; ?>
        </div>
        <?php endif; ?>
...
async function processarEnvios(lista) {
    ...
    for (let i = 0; i < lista.length; i++) {
        ...
        try {
            const res = await $.ajax({ url: 'index.php', method: 'POST', data: { ajax_send: true, contato }, dataType: 'json' });
...
$(document).ready(() => {
    ...
    $('#btn-todos').on('click', () => {
        if (confirm('Enviar para TODOS os <?= $total_registros ?> registros?')) {
            $.post('index.php', { get_all_ids: true }, (data) => processarEnvios(data));
        }
    });
